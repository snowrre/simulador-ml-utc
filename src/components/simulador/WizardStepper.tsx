...
const steps = [
    { title: 'Dataset', path: '/dataset', icon: (
        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12" /></svg>
    )},
    { title: 'Limpieza', path: '/preprocesamiento', icon: (
    ...
    { title: 'Resultados', path: '/comparativa', icon: (
...
export default function WizardStepper() {
    ...
    // Adjust the path for 'Modelo' dynamically
    const dynamicSteps = steps.map(step => {
        if (step.title === 'Modelo' && state.selectedProject) {
            return { ...step, path: `/modelo/${state.selectedProject}` };
        }
        return step;
    });

    const currentStepIndex = dynamicSteps.findIndex(step => pathname.includes(step.path));
        ...
        <div className="w-full max-w-4xl mx-auto mb-12">
            <div className="relative flex justify-between items-center">
                ...
                <div className="absolute left-0 top-1/2 -translate-y-1/2 w-full h-1 bg-gray-100 dark:bg-gray-800 -z-10 rounded-full overflow-hidden">
                    <motion.div 
                        initial={{ width: 0 }}
                        animate={{ width: `${(currentStepIndex / (dynamicSteps.length - 1)) * 100}%` }}
