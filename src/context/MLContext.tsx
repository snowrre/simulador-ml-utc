...
export interface MLDataState {
    ...
    taskType: 'classification';
    selectedProject: 'knn' | 'arbol' | null;
...
interface MLContextType {
    ...
    setTaskType: (type: 'classification') => void;
    setSelectedProject: (project: 'knn' | 'arbol' | null) => void;
...
export function MLProvider({ children }: { children: ReactNode }) {
    ...
    const setTaskType = (type: 'classification') => {
        setState(prev => ({ ...prev, taskType: type }));
    };

    const setSelectedProject = (project: 'knn' | 'arbol' | null) => {
    ...
    const applyPreprocessing = (): { trainX: number[][], trainY: any[], testX: number[][], testY: any[] } | null => {
        ...
        // We need to handle categorical features if they exist (simple ordinal encoding for now)
        ...
        state.featureColumns.forEach(colName => {
            ...
            if (colMeta?.type === 'categorical') {
                ...
                uniqueVals.forEach((v, i) => featureEncoders[colName][v] = i);
            }
        });
        ...
        const X = processedData.map((row: any) =>
            state.featureColumns.map(colName => {
                ...
                if (featureEncoders[colName]) {
                    return featureEncoders[colName][val];
                }
                return Number(val) || 0;
            })
        );

        const y = processedData.map((row: any) => row[state.targetColumn!]);
