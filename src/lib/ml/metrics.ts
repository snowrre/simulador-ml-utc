...
export function calculateClassificationMetrics(yTrue: any[], yPred: any[]): ClassificationMetrics {
    ...
    const labels = Array.from(new Set([
        ...yTrue.map(y => String(y).trim()), 
        ...yPred.map(y => String(y).trim())
    ]));
    ...
    let correct = 0;
    for (let i = 0; i < n; i++) {
        // Robust comparison with trim and normalization
        if (String(yTrue[i]).trim() === String(yPred[i]).trim()) correct++;
    }
    ...
    for (let i = 0; i < n; i++) {
        ...
        // Find matching label using robust comparison with trim
        const trueIdx = labels.findIndex(l => l === String(trueLabel).trim());
        const predIdx = labels.findIndex(l => l === String(predLabel).trim());
