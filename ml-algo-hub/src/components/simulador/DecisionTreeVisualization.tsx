...
const TreeNode = ({ label, subLabel, isLeaf, depth }: TreeNodeProps) => (
    <motion.div 
        ...
        className={`p-6 rounded-[2.5rem] border-2 flex flex-col items-center min-w-[220px] shadow-2xl relative bg-white dark:bg-gray-900 transition-all hover:scale-110 z-10 group/node ${
            isLeaf 
                ? 'border-emerald/40 text-emerald-600 dark:text-emerald-400' 
                : 'border-tech-blue/40 text-tech-blue-600 dark:text-tech-blue-400'
        }`}
    >
        <div className={`absolute -top-3 px-4 py-1 rounded-full text-[10px] font-black uppercase tracking-widest border shadow-lg ${
            isLeaf ? 'bg-emerald text-white border-emerald/20' : 'bg-tech-blue text-white border-tech-blue/20'
        }`}>
            {isLeaf ? 'Resultado' : 'División'}
        </div>
        <span className="text-sm font-black text-center mt-3 leading-tight uppercase italic tracking-tighter dark:text-white">{label}</span>
        {subLabel && (
            <div className="mt-3 pt-3 border-t border-gray-100 dark:border-white/5 w-full flex justify-center">
                <span className="text-[10px] font-black opacity-60 uppercase tracking-widest italic dark:text-gray-400">{subLabel}</span>
            </div>
        )}
        <div className="absolute -bottom-2 -right-2 w-6 h-6 bg-gray-100 dark:bg-gray-800 rounded-lg flex items-center justify-center border border-gray-200 dark:border-white/10 opacity-0 group-hover/node:opacity-100 transition-opacity">
            <span className="text-[8px] font-black text-gray-400">D{depth}</span>
        </div>
    </motion.div>
);
    ...
    const renderNode = (node: any, depth: number = 0): React.ReactNode => {
        ...
        return (
            <div className="flex flex-col items-center relative">
                ...
                <div className="flex gap-16 mt-12 relative w-full justify-center">
                    <div className="relative flex-1 flex flex-col items-center max-w-[300px]">
                        <div className="absolute top-[-48px] right-0 w-full h-12">
                    ...
                    <div className="relative flex-1 flex flex-col items-center max-w-[300px]">
                        <div className="absolute top-[-48px] left-0 w-full h-12">
                             <svg className="w-full h-full overflow-visible">
                                <motion.line 
                                    initial={{ pathLength: 0 }}
                                    animate={{ pathLength: 1 }}
            ...
            >
                {({ zoomIn, zoomOut, resetTransform }: { zoomIn: any, zoomOut: any, resetTransform: any }) => (
                    <>
                        ...
                        <div className="w-full flex justify-between items-center mb-12 relative z-10 bg-gray-50/50 dark:bg-white/5 p-4 rounded-2xl border border-gray-100 dark:border-white/5">
                            <div>
                                <h4 className="text-[10px] font-black text-gray-500 uppercase tracking-widest italic">Visualización Dinámica de Estructura</h4>
                                <p className="text-[9px] text-gray-400 font-bold uppercase tracking-tighter mt-1">Usa el mouse para arrastrar y zoom para explorar niveles</p>
                            </div>
                            <div className="flex gap-6">
                                <div className="flex items-center gap-2">
                                    <div className="w-2.5 h-2.5 rounded-full bg-tech-blue shadow-lg shadow-tech-blue/40"></div>
                                    <span className="text-[9px] font-black uppercase text-gray-500/80">Regla de Decisión</span>
                                </div>
                                <div className="flex items-center gap-2">
                                    <div className="w-2.5 h-2.5 rounded-full bg-emerald shadow-lg shadow-emerald/40"></div>
                                    <span className="text-[9px] font-black uppercase text-gray-500/80">Predicción Final</span>
                                </div>
                            </div>
                        </div>
